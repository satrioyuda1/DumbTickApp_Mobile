...
export default class EventDetail extends Component {
	...
	componentDidMount() {
		...
		Axios.get(`http://192.168.1.36:5000/api/v1/event/ ${params.eventId}`)
			.then((res) => {
						...
						<CardItem style={{ display: 'flex' }}>
							<Left style={{ flex: 1 }}>
								<View>
									...
									{/* <Text style={{ color: 'red' }}>{this.state.event.category}</Text> */}
								</View>
							</Left>
							<Right style={{ flex: 1 }}>
								<Button danger>
									<Text>{this.state.event.price}</Text>
								</Button>
							</Right>
						</CardItem>
						<CardItem>
							{/* <View style={{ flex: 1 }}>
								<Text>Hosted By</Text>
								<Text>{this.state.event.user.name}</Text>
							</View> */}
